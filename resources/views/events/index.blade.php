                            ...
                            <table id="eventTable" class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-primary update-btn" data-event-id="{{ $event->id }}" data-toggle="modal" data-target="#updateEventModal{{ $event->id }}">
                                                Update
                                            </button>
                                            <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#deleteEventModal{{ $event->id }}">
                                                Delete
                                            </button>
                                        </td>
                                    </tr>
...
@foreach($events as $event)
    <!-- Update Card Modal for card with ID {{ $event->id }} -->
    <div class="modal fade" id="updateEventModal{{ $event->id }}" tabindex="-1" role="dialog" aria-labelledby="updateEventModalLabel{{ $event->id }}" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <!-- Modal body -->
                <div class="modal-body">
                    <form id="updateEventForm{{ $event->id }}" action="{{ route('event.update', ['id' => $event->id]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('Post')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="name{{ $event->id }}" name="name" value="{{ $event->name }}">
                        </div>
                        <div class="form-group">
                            <label for="date{{ $event->id }}">Date</label>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                    <button type="button" class="btn btn-primary" onclick="updateEvent({{ $event->id }})">Save changes</button>
...
@foreach($events as $event)
    ...
    <div class="modal fade" id="deleteEventModal{{ $event->id }}" tabindex="-1" role="dialog" aria-labelledby="deleteEventModalLabel{{ $event->id }}" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="deleteEventLabel{{ $event->id }}">Confirm Deletion</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                ...
                <div class="modal-footer">
                    ...
                    <!-- Delete Form -->
                    <form id="deleteForm{{ $event->id }}" action="{{ route('event.delete', ['event' => $event->id]) }}" method="POST">
                        @csrf
                        @method('DELETE')
...
<script>
    function deleteEvent(eventId) {
        $.ajax({
            url: "{{ route('event.delete', ['event' => $event->id]) }}",
            ...
            success: function(response) {
                ...
                $('#deleteModal' + eventId).modal('hide'); // Close modal after successful delete
...
<script>
    function updateEvent(eventId) {
        ...
        $.ajax({
            url: "{{ route('event.update', ':id') }}".replace(':id', eventId), // Corrected URL
            method: 'Post',
            data: formData,
            processData: false,
            contentType: false,
