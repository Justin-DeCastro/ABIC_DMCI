        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#addAlderModal">
                                Add Properties
                            </button>
                        </div>
                        <div class="card-body">
                            <table id="cityTable" class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td><img src="{{ asset($alder->image) }}" alt="mid Image" width="100"></td>
                                        {{-- <td><img src="{{ asset('images/' . $alder->image) }}" alt="alder Image" width="100"></td> --}}
                                        <td>{{ $alder->name }}</td>
                                        <td>{{ $alder->price }}</td>
                                        <td>{{ $alder->description }}</td>
                                        <td>{{ $alder->status }}</td>

                                        <td>{{ $alder->units }}</td>
                                        <td>{{ $alder->location }}</td>

                                        <td>
                                            <button type="button" class="btn btn-primary update-btn" data-alder-id="{{ $alder->id }}" data-toggle="modal" data-target="#updateAlderModal{{ $alder->id }}">
                                                Update
                                            </button>
                                            <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#deleteAlderModal{{ $alder->id }}">
                                                Delete
                                            </button>
                                        </td>
                                    </tr>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form id="addAlderForm" action="{{ route('alder.store') }}" method="POST" enctype="multipart/form-data">
                                @csrf
            ...
            <div class="modal-body">
                ...
                <form id="updateAlderForm{{ $alder->id }}" action="{{ route('alder.update', ['id' => $alder->id]) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="price{{ $alder->id }}" name="description" value="{{ $alder->description }}">
                    </div>
...
@endforeach
        ...
        @foreach($alders as $alder)
            ...
            <!-- Delete Modal -->
            <div class="modal fade" id="deleteAlderModal{{ $alder->id }}" tabindex="-1" role="dialog" aria-labelledby="deleteModalLabel{{ $alder->id }}" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="deleteModalLabel{{ $alder->id }}">Confirm Deletion</h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        ...
                        <div class="modal-footer">
                            ...
                            <!-- Delete Form -->
                            <form id="deleteForm{{ $alder->id }}" action="{{ route('alder.delete', ['alder' => $alder->id]) }}" method="POST">
                                @csrf
                                @method('DELETE')
        ...
        <script>
            function deleteAlder(alderId) {
                $.ajax({
                    url: "{{ route('alder.delete', ['alder' => $alder->id]) }}",
